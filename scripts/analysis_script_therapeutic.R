...
metadata$Timepoint = factor(metadata$Timepoint, levels = c("FMT", "D0", "D42"))
...
alpha_diversity %>%
  ...
  capture.output(.,file = "stats_therapeutic_arm/alpha_div_chao1.txt")
...
alpha_diversity %>%
  ...
  capture.output(.,file = "stats_therapeutic_arm/alpha_div_simps.txt")
...
alpha_diversity %>%
  ...
  capture.output(.,file = "stats_therapeutic_arm/alpha_div_shan.txt")
...
write.csv(genus.glm, file = "stats_therapeutic_arm/genus_lv_lmer_stats.csv")

#hist(genus.glm$`anovas.Treatment:Timepoint Pr(>F).BH`)

#~ 11 significant interactions in the genus-level data
...
write.csv(GBMs.glm, file = "stats_therapeutic_arm/GBMs_lv_lmer_stats.csv")


#hist(GBMs.glm$`anovas.Treatment:Timepoint Pr(>F).BH`)


GBMBH <-  GBMs.exp[GBMs.glm[GBMs.glm$`anovas.Treatment:Timepoint Pr(>F).BH` < 0.2,"feature"],]
...
write.csv(GMMs.glm, file = "stats_therapeutic_arm/GMMs_lv_lmer_stats.csv")
